<?php
	...
	*/
	...
	if($_SESSION["$url_rest_page_show_graph_legend"] == "True"){
		$url_legend_export = "http://". $_SERVER["SERVER_NAME"] . "/" . $web_app_dir_http_prefix . "/Export.php?q=".urlencode($node_name)."&type=legend".commonUrlCustomizationValues();
		$legend_export = file_get_contents($url_legend_export);
		echo $legend_export . "<br />";
	}
	
	// attributes
	$url_attributes_export = "http://". $_SERVER["SERVER_NAME"] ."/" . $web_app_dir_http_prefix . "/Export.php?q=".urlencode($node_name)."&type=attributes".commonUrlCustomizationValues();
	...
	// image
	$url_html_export = "http://". $_SERVER["SERVER_NAME"] . "/" . $web_app_dir_http_prefix . "/Export.php?q=".urlencode($node_name)."&amp;type=html".commonUrlCustomizationValues($for_html=True);
