<?php
	...
	function getCategoryRuleFilterArray(){
		global $perspective_category_reference_rules;
		return $perspective_category_reference_rules;
	...
	$temp_category_show   	= $_SESSION["$url_rest_custom_image_category_show"];
	//echo "$temp_category_show";
	...
	if($export_type == "img" or $export_type == "html" or $export_type == "dot"){
		...
		if($temp_category_show == "False"){
			$matched_category_ereg = False;
			foreach($perspective_category_eregs as $regex){
				if(ereg($regex,$node_name) != False){
					$matched_category_ereg = True;
					break;
				}
			}
			if($matched_category_ereg){
				$filter_array = null;
			} else {
				$filter_array = getCategoryRuleFilterArray();
			}
		...
		$g 	= new GraphObject($query_runner,
				...
				$temp_graph_direction,$rule_filter=$filter_array);
