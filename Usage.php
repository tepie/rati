<?php
	...
	if(isset($_GET["$url_rest_node_param"])){
		$look_at_user = $_GET["$url_rest_node_param"];
	...
	if(isset($_GET["recent"])){
		$look_at_limit = $_GET["recent"] + 0;
		if($look_at_limit <= 0 or $look_at_limit < 100){
			$look_at_limit = 10;
		}
	}
	...
	$recent_searches 		= $watcher->recentSearches($limit=$look_at_limit);
	$recent_objects			= $watcher->recentNavigated($limit=$look_at_limit);
	$recent_users			= $watcher->recentUsers($limit=$look_at_limit);
	...
	$recent_user_searches   = $watcher->recentUserSearches($current_user,$limit=$look_at_limit);
	$recent_user_objects 	= $watcher->recentUserNavigated($current_user,$limit=$look_at_limit);
?>
<br />
