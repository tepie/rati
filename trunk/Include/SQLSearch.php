...
$search_index_structure_perspective='%s' where $search_index_structure_object = '%s'";
	...
	$search_index_exact_single_match = "SELECT $search_index_structure_object,$search_index_structure_combined_attributes, $search_index_structure_perspective FROM `$search_index_structrue_name` 
WHERE $search_index_structure_object = \"%s\" limit 1";

	$search_index_exact_perspective_single_match = "SELECT $search_index_structure_object,$search_index_structure_combined_attributes, $search_index_structure_perspective FROM `$search_index_structrue_name` 
WHERE $search_index_structure_object = \"%s\" and $search_index_structure_perspective = '%s' limit 1";

	$search_index_full_search = "SELECT $search_index_structure_object,$search_index_structure_combined_attributes, $search_index_structure_perspective FROM `$search_index_structrue_name` 
WHERE $search_index_structure_combined_attributes like ('%s') collate latin1_general_ci 
order by $search_index_structure_weight desc,$search_index_structure_rank desc limit %s,%s";

	$search_index_perspective_full_search = "SELECT $search_index_structure_object ,$search_index_structure_combined_attributes, $search_index_structure_perspective FROM `$search_index_structrue_name` 
WHERE $search_index_structure_combined_attributes like ('%s') collate latin1_general_ci and 
$search_index_structure_perspective = '%s' order by $search_index_structure_weight desc,$search_index_structure_rank desc limit %s,%s";
