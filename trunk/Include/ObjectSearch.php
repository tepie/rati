	...
	class SearchObject{
		...
		*/
		...
		private function executePerspectiveSearch($search_string,$perspective,$li_lower,$li_upper){
			global $search_index_exact_perspective_single_match;
			global $search_index_structure_object;
			global $search_index_structure_combined_attributes;
			global $search_index_structure_perspective;
			global $search_index_perspective_full_search;
			...
			$exact_sql = sprintf($search_index_exact_perspective_single_match,$string,$escaped_p);
			...
			if(isset($line[$search_index_structure_object])){
				//$return[$line["object"]] = $line["rule"];
				
				$return[$line[$search_index_structure_object]] = array();
				$return[$line[$search_index_structure_object]][$search_index_structure_combined_attributes] 		= $line[$search_index_structure_combined_attributes];
				$return[$line[$search_index_structure_object]][$search_index_structure_perspective] = $line[$search_index_structure_perspective];
				
			}	
			
			mysql_free_result($exact_res);
			
			$sql = sprintf($search_index_perspective_full_search,"%$escaped%",$escaped_p,"$li_lower","$li_upper");
		...
		private function executeSearch($search_string,$li_lower=0,$li_upper=30){
			global $search_index_exact_single_match;
			global $search_index_full_search;
			global $search_index_structure_perspective;
			global $search_index_structure_object;
			global $search_index_structure_combined_attributes;
			...
			$exact_sql = sprintf($search_index_exact_single_match,$string);
			//$exact_sql = "SELECT object_name as object,combined_attributes as rule, perspective FROM `search_index` ";
			...
			$sql = sprintf($search_index_full_search,"%$escaped%","$li_lower","$li_upper");
